<template>
  <b-container>
    <b-col>
      <div
        class="d-flex flex-wrap flex-md-row flex-column justify-content-sm-center justify-content-lg-around align-items-center"></div>
      <div class="col-12" v-if="posts">
        <b-card class="mb-2 item" id="products" v-for="post in posts" :key="post.id" tag="article"
                style="max-width: 20rem;"><img :src="imgUrl(post.image)" alt="Card image"/>
          ...
          <b-card-sub-title class="mb-3"><a href="#">{{post.category}}</a></b-card-sub-title>
          <b-card-text>{{post.short_text}}</b-card-text>
    ...
    <v-pagination v-if="paginationCount &gt; 1" v-model="currentPage" :page-count="paginationCount"
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      paginationCount: (state) => state.blog.paginationCount
    }),
    ...mapState('blog', [
      '_myData'
    ])
